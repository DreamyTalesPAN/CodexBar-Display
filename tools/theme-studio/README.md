...
The MVP targets the Mini theme only. ThemeSpec metadata is kept minimal, and the UI exposes the stable theme name instead of internal revision controls. Legacy `fallbackTheme` and `fb` fields are accepted during import but omitted from normalized and exported specs.
...
Use `Save Theme` to keep the current ThemeSpec in the repo workspace and reuse it after restarting the dev server. In static builds without the dev API, it falls back to browser storage only.
