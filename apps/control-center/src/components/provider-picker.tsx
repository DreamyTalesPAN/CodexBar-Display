...
export function providerSetupCanFinish(
  items: PreferenceDescriptor[] | null,
  display: ProviderDisplaySelection | null,
  pendingPreferenceIds: Set<string>,
  pendingCheckIds: Set<string>,
  ...
  if (enabled.some((item) => !selected.has(item.providerId))) {
    return false;
  }
  return enabled.every((item) => providerSetupItemPasses(item, now));
}

// "healthy" requires a fresh live verification. "stale" means this provider
// already produced a real successful reading before; a momentary live
// re-check miss (e.g. under system load) must not block setup completion
// when the provider is demonstrably working. Every other state
// (auth_required, setup_required, no_usage_available, timeout,
// config_error, unavailable without a prior success, etc.) still requires
// the customer to act (or re-check) before finishing.
export function providerSetupItemPasses(
  item: Pick<ProviderItem, "health">,
  now = Date.now(),
): boolean {
  if (item.health.state === "healthy") {
    return providerVerificationIsFresh(item.health.verifiedAt, now);
  }
  return item.health.state === "stale";
}

// Providers that are enabled, included in the current display selection,
// and not currently passing the setup check. These are exactly the
// providers a "Check again" action should re-verify: re-running the check
// is the one thing a customer can do themselves without leaving Setup
// (e.g. after a transient timeout under system load), as opposed to
// providers missing from the display selection entirely, which need a
// selection change instead.
export function providerSetupBlockingProviders(
  items: PreferenceDescriptor[] | null,
  display: ProviderDisplaySelection | null,
  now = Date.now(),
): ProviderItem[] {
  ...
  return (items || [])
    ...
    .filter(
      (item) =>
        item.value &&
        selected.has(item.providerId) &&
        !providerSetupItemPasses(item, now),
    );
}
