...
export type ProviderPickerProps = {
  ...
  onRecovery: (item: PreferenceDescriptor) => void | Promise<void>;
  setupMode?: boolean;
};

type ProviderItem = PreferenceDescriptor & {
  type: "boolean";
  value: boolean;
  providerId: string;
        ...
        {items === null && !preferencesError ? (
          ...
          <fieldset className="grid gap-3">
            ...
            {providers.map((item) => {
              ...
              const recovery = item.health.nextAction;
              return (
                <div
                ...
                >
                  <div className="min-w-0">
                    ...
                    >
                      {pendingCheck ? "Checking provider." : item.health.message}
                    </p>
                    {recovery && item.health.recoveryAction && !pendingCheck ? (
                      <div className="mt-3 grid justify-items-start gap-2">
                        ...
                        <Button
                          className="min-h-11"
                          onClick={() => void onRecovery(item)}
                          size="sm"
                          type="button"
                          variant="outline"
                        >
                          <Settings2 aria-hidden /> Open recovery
                        </Button>
                      </div>
                    ) : null}
                    {disableLocked ? (
