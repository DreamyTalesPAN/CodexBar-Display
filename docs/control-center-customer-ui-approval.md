...
- Scope: Revert squash commit `bb7dfa45` in full, then republish the two affected theme packs at a higher version and ThemeSpec revision so no published asset is deleted. Outside `theme-packs/` and `dist/theme-packs/`, the resulting tree is identical to `db281000`. No release, installation, or device operation. The withdrawn firmware and Control Center work stays on `codex/integration-p1-batch` for hardware verification.
