...
## 2026-07-22 — Customer Service chat and reply email

- User approval: The user explicitly ordered the Customer Service chat implementation in the Codex task on 2026-07-22, then reviewed it in the local Control Center and explicitly requested the reply-email field, removal of the `optional` label, and corrected chat-input spacing and borders.
- Approved customer-visible result: A floating `Customer Service` action opens the `VibeTV Support` sheet, full-screen in small windows, with the support greeting, an `Email for replies` field without an `optional` suffix, the chat history and message composer, `New conversation`, the AI-support safety notice, email support, and Privacy Policy links. The reply email is retained only for the current local conversation and clears with `New conversation`. The composer uses balanced vertical spacing, a quiet rounded border, and one clear focus state.
- Approved files: `control-center-app.tsx`, `customer-service-chat.tsx`, `support-chat.ts`, `support-chat.test.ts`, `globals.css`, and the matching customer-flow assertions in `test-customer-flows.mjs`.
