...
- Claude Creature: original idle and working CBA artwork remains animated at
  3 and 6 fps. The additional needs-you, done, and error SVGs from Claude Design
  include provenance metadata but no animation frames. Their bodies are enlarged
  within the existing 77x77 slot to match the original Creature's visual size;
  each remains a static state pose. No additional footer replaces reset information.
