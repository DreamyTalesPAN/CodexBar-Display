...
- Reviewed scope: Overview setup flow, setup-locked navigation, Theme Library install readiness, Updates Mac App actions, Support report, Timeline events, Mac App installer download copy, and customer release API download states.
...
- Simplifications accepted: setup recovery stays on Overview; Theme Library stays locked until setup is complete; Support report uses `VibeTV address` instead of `Target`; Timeline events use customer language; installer download copy says `downloaded installer` instead of package internals; script-only Companion releases expose no customer install link; customer UI says `Mac App` instead of the internal Companion service name.
- Verification: customer-flow browser tests cover setup locks, desktop header state, Updates action states, Support report exports, Timeline copy, setup-jargon guards, customer release API download states, and mobile overflow; `check:customer-ui-copy` blocks internal customer-facing wording; lint and customer-ready gate local checks run green.
