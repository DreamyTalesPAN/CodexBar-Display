...
- `GET /health` returns a top-level `standby` object with the live state: `active` and `idleSecs`. Standby starts after `timeoutMinutes` without a frame that moved the usage numbers and ends on the first frame that does. Standby brightness applies on entry and the normal brightness is restored on wake. Both transitions reload the ThemeSpec from LittleFS, so a transition never writes flash and the device returns to the live theme after a reboot.
