...
- Connected devices expose `http://vibetv.local` with mDNS, log the fallback IP, show `app.vibetv.shop` on the display, serve a local setup hub that links to the hosted Control Center, and wait for the Mac App.
