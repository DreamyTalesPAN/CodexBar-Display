...
- After shrink changes size, use `valign` / `va` (`middle`/`center` or `bottom`) so the glyphs stay in the lane. Set `h` to the box (for example a 32px logo); omit `h` to use the max `fontSize` lane (`ApproxTextHeight`). Omit `va` to keep today's top alignment. `y` is the top of that box. `va` is not compatible on older firmware (it keeps top alignment), so packs that emit it must declare `text-valign-v1` and `minFirmware` 1.0.42. Write the exact tokens `middle`, `center`, or `bottom` — firmware does not lowercase aliases.
