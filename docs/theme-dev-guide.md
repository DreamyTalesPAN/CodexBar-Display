...
- Token bindings may use `numberFormat: "compact"` (`nf`) for values such as `142.4M` and `animation: "count-up"` (`an`) for a 12-step upward transition. Decreases and cold boots render the target immediately.
