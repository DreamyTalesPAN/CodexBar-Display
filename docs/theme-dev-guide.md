...
- Token text binds directly to the values already delivered by CodexBar: `sessionTokens` is today, `weekTokens` is the last 7 days, and `totalTokens` is the last 30 days. ThemeSpec does not recalculate these periods.
- Token bindings may use `numberFormat: "compact"` (`nf`) for values such as `142.4M`. `animation` (`an`) supports `count-up` for numeric interpolation and `slot-roll` for an old-value-out/new-value-in vertical transition. Decreases and cold boots render the target immediately.
