...
- Use `{sessionLabel}` / `{sl}` and `{weeklyLabel}` / `{wl}` for provider-aware quota names. Use `{resetStatus}` / `{rs}` when the complete reset line should become `Reset unavailable` instead of `Reset in 0m`. Existing standard-theme text beginning with `Session` or `Weekly` is mapped to provider-aware labels by firmware for backward compatibility.
