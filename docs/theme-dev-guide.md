...
- A text primitive whose only substitution is an unavailable countdown collapses to `Reset unavailable` as a whole, so hard-coded prose around it disappears instead of doubling up. `Resets in {usageSlot1Reset}` renders as `Reset unavailable`, never `Resets in Reset unavailable`. This matters for idle windows: a Claude account with no active session reports a real 0% with no reset time at all. A template that also substitutes a label or percentage keeps substituting in place, so `{usageSlot1Label} {usageSlot1Reset}` still renders `Session Reset unavailable`. Put the countdown in its own primitive and give the prose a separate one (as `reset-countdown` and `tiny-office` do) when the label must stay visible.
