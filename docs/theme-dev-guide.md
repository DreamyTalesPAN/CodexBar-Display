...
- After shrink changes size, use `valign` / `va` (`middle`/`center` or `bottom`) so the glyphs stay in the lane. Set `h` to the box (for example a 32px logo); omit `h` to use the max `fontSize` lane (`ApproxTextHeight`). Omit `va` to keep today's top alignment. `y` is the top of that box.
