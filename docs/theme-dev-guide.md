...
- A countdown with no deadline renders one of two strings, and the difference is what the device can stand behind. `Reset unavailable` means the basis went stale or offline, so any number would be a guess. `No active session` means the window is current and measured but has nothing scheduled to reset: a Claude account with no session started reports a real 0% with no reset time at all, and reporting that as unavailable made a healthy account look broken. A countdown that merely ran out keeps `Reset unavailable` until the next frame carries the new deadline.
- A text primitive whose only substitution is such a countdown collapses to that string as a whole, so hard-coded prose around it disappears instead of doubling up. `Resets in {usageSlot1Reset}` renders as `No active session` for an idle window, never `Resets in Reset unavailable`. A template that also substitutes a label or percentage keeps substituting in place, so `{usageSlot1Label} {usageSlot1Reset}` still renders `Session No active session`. One template binding both an idle and an untrustworthy countdown keeps `Reset unavailable`; it cannot claim everything is merely idle. Put the countdown in its own primitive and give the prose a separate one (as `reset-countdown` and `tiny-office` do) when the label must stay visible.
