...
func TestDMGRuntimeBlocksUnsafeESP8266FirmwareBeforeFetchOrSend(t *testing.T) {
	prepareFastTestEnv(t)
	t.Setenv(minimumSafeFirmwareEnvVar, "1.0.36")

	var fetched, sent int
	err := runCycleWithDeps(context.Background(), "http://192.168.178.72", &runtimeState{}, runtimeDeps{
		transportName: "wifi",
		resolvePort: func(target string) (string, error) {
			return target, nil
		},
		...
		fetchUpdateState: func(context.Context, protocol.DeviceCapabilities) (protocol.UpdateState, error) {
			return protocol.UpdateState{}, nil
		},
		logf: func(string, ...any) {},
		sendLine: func(string, []byte) error {
			sent++
	...
	if err == nil {
		t.Fatal("expected old ESP8266 firmware to be blocked")
	}
	runtimeErr := asRuntimeError(err)
	if runtimeErr.Kind != runtimeErrorFirmwareSafety || runtimeErr.Op != "firmware-safety-check" {
		t.Fatalf("unexpected safety error: %+v", runtimeErr)
	}
	if !strings.Contains(runtimeErr.RecoveryAction(), "1.0.36 or newer") {
		t.Fatalf("expected required firmware in recovery action, got %q", runtimeErr.RecoveryAction())
	}
	if fetched != 0 || sent != 0 {
		t.Fatalf("unsafe firmware must be blocked before usage fetch or frame send: fetched=%d sent=%d", fetched, sent)
	}
}

func TestDMGRuntimeFirmwareSafetyGateAllowsRepairedAndUnscopedDevices(t *testing.T) {
	tests := []struct {
		name          string
		minimum       string
		...
		{name: "repaired ESP8266 firmware", minimum: "1.0.36", transportName: "wifi", board: "esp8266-smalltv-st7789", firmware: "1.0.36"},
		{name: "newer ESP8266 firmware", minimum: "1.0.36", transportName: "wifi", board: "esp8266-smalltv-st7789", firmware: "1.0.37"},
		{name: "unverifiable ESP8266 firmware", minimum: "1.0.36", transportName: "wifi", board: "esp8266-smalltv-st7789", firmware: "unknown", wantBlocked: true},
		{name: "other WiFi board", minimum: "1.0.36", transportName: "wifi", board: "esp32-lilygo-t-display-s3", firmware: "1.0.35"},
		{name: "legacy runtime without DMG gate", transportName: "wifi", board: "esp8266-smalltv-st7789", firmware: "1.0.35"},
		{name: "USB runtime", minimum: "1.0.36", transportName: "usb", board: "esp8266-smalltv-st7789", firmware: "1.0.35"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv(minimumSafeFirmwareEnvVar, tt.minimum)
			err := requireSafeWiFiFirmware(protocol.DeviceCapabilities{
				Board:    tt.board,
				Firmware: tt.firmware,
			}, runtimeDeps{transportName: tt.transportName})
			if (err != nil) != tt.wantBlocked {
				t.Fatalf("blocked=%v want %v: %v", err != nil, tt.wantBlocked, err)
			}
		})
	}
}
