...
void SendSetupPage(
  ...
  if (supportUrl != nullptr && supportUrl[0] != '\0') {
    String help;
    help.reserve(strlen(supportUrl) + 420);
    help += F("<a class=\"help\" href=\"");
    help += HtmlEscape(String(supportUrl));
    help += F("\" target=\"_blank\" rel=\"noopener noreferrer\" aria-controls=\"support-note\" aria-expanded=\"false\" onclick=\"if(location.hostname==='captive.apple.com'){event.preventDefault();this.setAttribute('aria-expanded','true');document.getElementById('support-note').hidden=false}\">My Wi-Fi isn't shown</a><p id=\"support-note\" class=\"support-note\" role=\"status\" hidden>Close this setup window and disconnect from VibeTV-Setup. Then open <strong>vibetv.shop/pages/setup</strong> in Safari.</p>");
