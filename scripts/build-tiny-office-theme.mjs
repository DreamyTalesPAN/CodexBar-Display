...
// the top 2 and bottom 10 rows (ceiling/floor) are dropped so the scene is
// 120x60 and the device can give the numbers a taller font. Drawn 2x.
const FRAME_H = 72, TOP_CUT = 2, SCENE_H = 60;
...
for (const y of [4, 35]) for (let x = 0; x < 60; x++) backdrop[y * 60 + x] = 1;
...
const manifest = {
  ...
  version: "0.3.0", minFirmware: "1.0.40", usage: "live", requiredCapabilities: ["usage-slots-v1"],
  themeSpec: {
